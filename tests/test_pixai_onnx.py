...
def _install_fake_modules(monkeypatch, outputs: list[np.ndarray]) -> None:
    class FakeSession:
        ...
        def run(
            self, _outputs: object, feed: dict[str, np.ndarray]
        ) -> list[np.ndarray]:
...
def _write_bundle(root: Path) -> None:
    ...
    (bundle / "thresholds.csv").write_text(
        "category,threshold\ngeneral,0.3\ncharacter,0.85\n", encoding="utf-8"
    )


def test_pixai_predict_returns_tag_predictions_from_local_bundle(
...
def test_pixai_missing_required_file_has_readable_error(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_pixai_unknown_preprocess_stage_fails_fast(tmp_path: Path, monkeypatch) -> None:
    ...
    bundle = tmp_path / "deepghs" / "pixai-tagger-v0.9-onnx"
    (bundle / "preprocess.json").write_text(
        '[{"type":"center_crop"}]', encoding="utf-8"
    )
    _install_fake_modules(monkeypatch, [np.asarray([[0.0]], dtype="float32")])
    monkeypatch.setenv("CCBTAG_MODEL_DIR", str(tmp_path))
    monkeypatch.setenv("CCBTAG_MODEL_SOURCE", "hf")
